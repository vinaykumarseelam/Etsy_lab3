...
function profileForm() {
  ...
  const [getUserById, { loading, data: userdata }] = useLazyQuery(
    GETUSER_BY_ID(user.id)
  );
  ...
  const handleUserData = (e) => {
    ...
    Axios.put(
      `${process.env.REACT_APP_ENDPOINT}/api/users/updateUser/` + user.id,
      formData
    ).then((response) => {
  ...
  useEffect(() => {
    if (userdata) {
      if (userdata.getUserById) {
        const data = userdata.getUserById;
        const response = {
          data: {
            user: data[0],
          },
        ...
        setProfileImageUrl(response.data.user["profilePic"]);
  ...
  const fetchItemDetails = (e) => {
    ...
    // Axios.get(
    //   `${process.env.REACT_APP_ENDPOINT}/api/users/getShopById/` + user.id
    // ).then((response) => {
            ...
            <form encType="multipart/form-data">
              ...
              <div className="section">
                ...
                >
                  ...
                  <option value="Daily Cityn Republic">
                    Daily Cityn Republic
                  </option>
