...
function shopHome() {
  ...
  const { loading, data } = useQuery(GET_MY_PRODUCTS(user.id));
  ...
  useEffect(() => {
    if (data) {
      if (data.getMyProducts) {
        setProducts(data.getMyProducts);
      }
    }
  }, [data]);
  ...
  var viewItems = (variables) => {
    ...
    setShowProds(true);
    Axios.post(
      `${process.env.REACT_APP_ENDPOINT}/api/products/getAllProducts/` +
        user.id,
      {
        searchTerm: variables["searchTerm"],
      }
    ).then((response) => {
      if (response.data.success) setProducts(response.data.result);
    });
  };
  ...
  const updateSearchTerm = (newSearchTerm) => {
    ...
    if (!newSearchTerm) return setProducts(data.getMyProducts);
    let prod = data.getMyProducts.filter((pro) => {
      let itemName = pro.itemName.toLowerCase();
      if (itemName.includes(newSearchTerm)) return pro;
    });
    setProducts(prod);
  ...
  return (
    <div>
      ...
      {showShowImageEditPage && (
        <EditItemImage
          setShowProductsEditPage={setShowShowImageEditPage}
          itemId={productId}
          viewItems={viewItems}
        />
      )}
